...
update:
    ...
    if (ofs_read(tdb, FREELIST_TOP, &rec->next) == -1 || rec_write(tdb, offset, rec) == -1 || ofs_write(tdb, FREELIST_TOP, &offset) == -1) {
...
#else
    if (lseek(tdb->fd, (size + addition) - 1, SEEK_SET) != (size + addition) - 1 || write(tdb->fd, &b, 1) != 1) {
#endif
    ...
    struct list_struct* rec)
{
    ...
    for (; tlock->hash < tdb->header.hash_size; tlock->hash++) {
        ...
        while (tlock->off) {
            ...
            current = tlock->off;
            tlock->off = rec->next;
            if (!tdb->read_only && do_delete(tdb, current, rec) != 0)
                goto fail;
        }
...
int tdb_append(TDB_CONTEXT* tdb, TDB_DATA key, TDB_DATA new_dbuf)
{
    ...
    if (rec.data_len && tdb_read(tdb, rec_ptr + sizeof(rec) + rec.key_len, p + key.dsize, rec.data_len, 0) == -1)
        goto fail;
    ...
    tdb_hash_func hash_fn)
{
    ...
    if ((tdb_flags & TDB_CLEAR_IF_FIRST) && (locked = (tdb_brlock(tdb, ACTIVE_LOCK, F_WRLCK, F_SETLK, 0) == 0))) {
        open_flags |= O_CREAT;
        if (ftruncate(tdb->fd, 0) == -1) {
    ...
    vertest = (((u32)vp[0]) << 24) | (((u32)vp[1]) << 16) | (((u32)vp[2]) << 8) | (u32)vp[3];
