 ...
 */
static const char*
    eap_state_name(esc) enum eap_state_code esc;
{
 ...
 */
static void
    eap_client_timeout(arg) void* arg;
{
 ...
 */
void
    eap_authwithpeer(unit, localname) int unit;
char* localname;
{
    eap_state* esp = &eap_states[unit];
...
static int
    b64enc(bs, inp, inlen, outp) struct b64state* bs;
u_char* inp;
int inlen;
u_char* outp;
{
...
static int
    b64flush(bs, outp) struct b64state* bs;
u_char* outp;
{
    int outlen = 0;
...
static int
    b64dec(bs, inp, inlen, outp) struct b64state* bs;
u_char* inp;
int inlen;
u_char* outp;
{
...
int status;
{
    ...
    case eapIdentify:
        ...
        if (get_srp_secret(esp->es_unit, esp->es_server.ea_peer,
                esp->es_server.ea_name, (char*)secbuf, 1)
            != 0) {
 ...
 */
void
    eap_authpeer(unit, localname) int unit;
char* localname;
{
    eap_state* esp = &eap_states[unit];
 ...
 */
static void
    eap_server_timeout(arg) void* arg;
{
    eap_state* esp = (eap_state*)arg;
...
static void
    srp_lwrechallenge(arg) void* arg;
{
    eap_state* esp = (eap_state*)arg;
        ...
        case EAPSRP_SKEY:
            ...
            if (esp->es_client.ea_skey != NULL) {
                ...
                if (get_srp_secret(esp->es_unit,
                        ...
                        esp->es_client.ea_peer, secret, 0)
                    == 0) {
                    /*
 ...
 */
static void
    eap_input(unit, inp, inlen) int unit;
u_char* inp;
int inlen;
{
    eap_state* esp = &eap_states[unit];
